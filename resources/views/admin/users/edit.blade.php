                    ...
                    <form class="form-horizontal" method="POST" action="{{ route('admin.users.update', $user->id) }}">
                        ...
                        <div class="flex items-stretch mb-3">
                            ...
                            <div class="flex flex-col w-3/4">
                                <input id="name" type="text" class="flex-grow h-8 px-2 border rounded {{ $errors->has('name') ? 'border-red-dark' : 'border-grey-light' }}" name="name" value="{{ old('name', $user->name) }}" autofocus>
                        ...
                        <div class="flex items-stretch mb-3">
                            ...
                            <div class="flex flex-col w-3/4">
                                <input id="email" type="email" class="flex-grow h-8 px-2 border rounded {{ $errors->has('email') ? 'border-red-dark' : 'border-grey-light' }}" name="email" value="{{ old('email', $user->email) }}" required>
                        ...
                        <div class="flex items-stretch mb-3">
                            ...
                            <div class="flex flex-col w-3/4">
                                <input id="mobile" type="number" class="flex-grow h-8 px-2 border rounded {{ $errors->has('mobile') ? 'border-red-dark' : 'border-grey-light' }}" name="mobile" value="{{ old('mobile', $user->mobile) }}" required>
                        ...
                        <div class="flex items-stretch mb-3">
                            ...
                            <div class="flex flex-col w-3/4">
                                <select name="role" class="block appearance-none w-full bg-white border border-grey-light hover:border-grey px-4 py-2 pr-8 rounded shadow" onchange="if( this.value == 'institute' || this.value == 'editor' ) { document.getElementById('institute_selector').style.display = '' } else { document.getElementById('institute_selector').style.display = 'none'}" >
                                    ...
                                    <option value="{{ $r_value }}" @if(old( 'role' , $user->role ) == $r_value ) selected @endif >{{ $role_name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="flex items-stretch mb-3" style="@if( ! $user->institute_id ) display: none @endif" id="institute_selector">
                            ...
                            <div class="flex flex-col w-3/4">
                                <select name="institute_id" class="block appearance-none w-full bg-white border border-grey-light hover:border-grey px-4 py-2 pr-8 rounded shadow" >
                                    ...
                                    <option value="{{ $institute_id }}" @if(old( 'institute_id' , $user->institute_id ) == $institute_id ) selected @endif>{{ $institute }}</option>
                                    @endforeach
                                </select>
                                {!! $errors->first('institute_id', '<span class="text-red-dark text-sm mt-2">:message</span>') !!}
                            </div>
                        </div>
                        @else
